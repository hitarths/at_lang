...
def get_left_right_for_equality(left, right, z3_vars_left, z3_vars_right = None):
    if not z3_vars_right: z3_vars_right = z3_vars_left

    if (left.data == "select"):
        left = get_select(left, z3_vars_left)
    else:
        #left must be a var
        left = z3_vars_left[left.children[0].value]
    if (right.data == "select"):
        right = get_select(right, z3_vars_right)
    elif (right.data == "var"):
        right = z3_vars_right[right.children[0].value]
    elif (right.data == "varnum"):
        if (right.children[0].data == "var"):
            right = z3_vars_right[right.children[0].children[0].value]
        else:
